    ...
    <div className="bg-white rounded-2xl border border-[var(--darkblue)]/10 shadow-sm overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-[var(--darkblue)]/10 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-[var(--background)] p-1 rounded-xl">
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${activeTab === "all"
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${activeTab === "temporary"
              ? "bg-honey text-white shadow-sm"
              : "text-[var(--darkblue)]/70 hover:bg-white hover:text-[var(--darkblue)]"
              }`}
          >
            <MonitorSmartphone className={`w-3.5 h-3.5 ${activeTab === "temporary" ? "text-white" : "text-black"}`} />
            Temporary ({tempDocsCount})
          </button>
          <button
            onClick={() => setActiveTab("permanent")}
            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${activeTab === "permanent"
              ? "bg-honey text-white shadow-sm"
              : "text-[var(--darkblue)]/70 hover:bg-white hover:text-[var(--darkblue)]"
              }`}
          >
            <HardDrive className={`w-3.5 h-3.5 ${activeTab === "permanent" ? "text-white" : "text-black"}`} />
            Cloud Saved ({permanentDocsCount})
          </button>
          <button
            onClick={() => setActiveTab("favorites")}
            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${activeTab === "favorites"
              ? "bg-honey text-white shadow-sm"
              : "text-[var(--darkblue)]/70 hover:bg-white hover:text-[var(--darkblue)]"
              }`}
          >
            <Star className={`w-3.5 h-3.5 ${activeTab === "favorites" ? "text-white fill-white" : "text-black"}`} />
            Starred
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center bg-[var(--background)] p-1 rounded-lg">
            <button
              ...
              className={`p-1.5 rounded-md transition-colors cursor-pointer ${viewMode === "list" ? "bg-white text-[var(--darkblue)] shadow-sm" : "text-[var(--darkblue)]/50 hover:text-[var(--darkblue)]"
                }`}
              title="List View"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-md transition-colors cursor-pointer ${viewMode === "grid" ? "bg-white text-[var(--darkblue)] shadow-sm" : "text-[var(--darkblue)]/50 hover:text-[var(--darkblue)]"
                }`}
              title="Grid View"
            >
      ...
      {docsLoading || authLoading ? (
        ...
        <div className="py-20 text-center flex flex-col items-center justify-center">
          ...
          <h3 className="text-sm font-extrabold text-[var(--primary)]">No documents found</h3>
              ...
              <tr className="bg-[var(--cream)]/80 text-[11px] font-extrabold text-[var(--primary)] uppercase tracking-wider border-b border-[var(--darkblue)]/10 select-none">
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    {sortField === "title" ? (
                      sortOrder === "asc" ? <ArrowUp className="w-3 h-3 text-black" /> : <ArrowDown className="w-3 h-3 text-black" />
                    ) : (
                ...
                >
                  ...
                  <td className="py-3.5 px-4">
                    {doc.type === "temporary" ? (
                      <span className="inline-flex items-center gap-1 text-[11px] font-medium text-[var(--darkblue)] bg-[var(--yellow)]/30 border border-slate-300 px-2.5 py-0.5 rounded-full">
                        <MonitorSmartphone className="w-3 h-3 text-black" /> Local Draft
                      </span>
                    ) : (
                  ...
                  <td className="py-3.5 px-4 text-right">
                    <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                      <button
                        onClick={() => {
                          if (doc.type === "temporary") handleToggleTempFavorite(doc.id);
                          else handleTogglePermanentFavorite(documents.find((d) => d.id === doc.id)!);
                        }}
                        className="p-1.5 rounded-lg hover:bg-slate-200 text-black transition-colors cursor-pointer"
                        title="Favorite"
                      >
                        <Star className={`w-4 h-4 text-black ${doc.is_favorite ? "fill-black" : ""}`} />
                      </button>
                      <button
                        onClick={() => {
                          if (doc.type === "temporary") handleRemoveTempDoc(doc.id);
          ...
          {displayedDocs.map((doc) => (
            ...
            >
              ...
              <div className="flex items-center justify-between text-[11px] text-[var(--darkblue)]/50 pt-3 border-t border-[var(--darkblue)]/5">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-[10px] text-[var(--darkblue)]/70 font-medium">{getFileSize(doc.content)}</span>
                  <span>•</span>
                  <span>{timeAgo(doc.updated_at)}</span>
                </div>
                <span className={`font-medium px-2.5 py-0.5 rounded-full text-[10px] ${doc.type === "temporary" ? "bg-[var(--yellow)]/30 text-[var(--darkblue)] border border-slate-300" : "bg-[var(--blue)]/30 text-[var(--darkblue)] border border-blue-200"}`}>
